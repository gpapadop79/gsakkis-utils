...
A regular expression just-for-fun recipe for leeching emails.. even
some of those that are supposed to be only human readable.

Well, this may look like an open invitation to spammers, but I guess
they'd have something similar - and probably more efficient - if they
really needed to.
'''
...
def test():
    text = \
        ''' You can contact us at myname@server.site.com
            ...
            and t.h.e.i.r.n.a.m.e at server dot s/i/t/e DOT C.O.M.
        '''
...
def emailLeech(text):
    ...
    while (True):
        ...
        # join the parts
        yield "%s@%s" % (parts[0], '.'.join(parts[1:]))
        text = text[match.end():]
