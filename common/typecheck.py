...
@var ENABLE_TYPECHECKING: If L{ENABLE_TYPECHECKING} is False, L{typecheck},
    L{expects} and L{returns} are no-ops. By default, L{ENABLE_TYPECHECKING}
    is False for optimized bytecode (selected with the '-O' command line flag).
    Importing modules may set this at will to control explicitly whether
    typechecking is to be performed.

    B{CAVEAT}: Changing L{ENABLE_TYPECHECKING} has no effect after a function
    has been decorated with L{expects} and/or L{returns}. On the other hand,
    L{typecheck} checks the current value of L{ENABLE_TYPECHECKING}, so
    typechecking can be turned on and off dynamically.

@sort: TypeCheck, instanceOf, typeIs, containerOf, recordOf, mappingOf,
       tupleOf, listOf, setOf, dictOf
@group Enforcing The Type Checks: typecheck, expects, returns
'''
...
__all__ = '''TypeCheck instanceOf typeIs
             containerOf recordOf mappingOf tupleOf listOf setOf dictOf
             typecheck expects returns ENABLE_TYPECHECKING
          '''.split()
...
def typecheck(vars=None, **checkedVars):
    ...
    This is a no-op if L{ENABLE_TYPECHECKING} is False.
...
def expects(**checkedArgs):
    '''Specify the expected argument types of a function.

    In python 2.4+, the typical use is in function decorators:
        >>> from typecheck import *
        >>> @expects(x=str, y=listOf(int))
        ... def f(x,y):
        ...     pass
        >>> f('1',[2,3])
        >>> f(1,[2,3])
...
class containerOf(TypeCheck):
    ...
    def __init__(self, itemCheck, size=None):
        ...
        '''
        self._itemCheck = _wrapTypeCheck(itemCheck)
        self._size = size
...
class recordOf(TypeCheck):
    ...
    def __init__(self, *itemChecks):
        ...
        @type itemChecks: Sequence of L{typechecks <TypeCheck>}, types or
            classes.
        '''
        self._itemChecks = map(_wrapTypeCheck,itemChecks)
...
class mappingOf(TypeCheck):
    ...
    def __init__(self, key=object, value=object):
        ...
        '''
        keyCheck,valueCheck = map(_wrapTypeCheck, (key,value))
        typesStr = ["*","*"]
        if value is object:
            # 1. check only keys
...
def _wrapTypeCheck(typeOrCheck):
    '''Wrap a L{TypeCheck}, type or class into a L{TypeCheck}.

    @param typeOrCheck: A L{TypeCheck}, type or class.
    @return: the argument if it is already a L{TypeCheck}; otherwise return
        instanceOf(typeOrCheck).
    @rtype: L{TypeCheck}
    '''
    return (isinstance(typeOrCheck, TypeCheck)
            and typeOrCheck
            or instanceOf(typeOrCheck))
